...
final class EventStoreInMemoryFactory
{
    public static function create(): EventStoreInMemory
    {
        ...
        for ($i = 0; $i < 10; ++$i) {
            ...
            $eventStore->append('foo', $i, StubEvent::class, $event, null, new \DateTimeImmutable('2018-11-14 19:42:'.($i * 2).'.1234'));
        }

        for ($i = 0; $i < 5; ++$i) {
            $event = new StubEvent($i * 3);
            $eventStore->append('bar', $i, StubEvent::class, $event, null, new \DateTimeImmutable('2018-11-14 19:42:'.($i * 2).'.1234'));
        }
